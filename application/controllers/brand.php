...
class Brand extends CI_Controller
{
	...
	public function index()
	{
		...
		$uri = $this->db->query('SELECT * FROM brand WHERE brand_name="'.$param.'" LIMIT 1')->row();
		$id = $uri->brand_id;
		$data['is_active'] = $param;

		if ($param!='') {
			if ($param_type=='' || is_numeric($param_type)) {
				$nav = $this->db->query('SELECT brand_id, brand_name FROM brand WHERE brand_name="'.$param.'" LIMIT 1')->row();
				...
				$data['list'] = $this->db->query('SELECT * FROM product WHERE brand_id="'.$nav->brand_id.'" && product_status="publish"'.$price.' '.$sort.' LIMIT '.$this->limit.' offset '.$offset)->result();
				$data['count'] = $this->db->query('SELECT * FROM product WHERE brand_id="'.$nav->brand_id.'" && product_status="publish"'.$price)->num_rows();
				if ($data['count']==0) {
				...
				$config['base_url'] = base_url().'brand/'.str_replace(" ", "-", strtolower($param)).'/';
				...
				$data['title'] = 'Koleksi Sepatu dan Tas '.ucfirst($param).' - Pusat Sepatu dan Tas Terlengkap';
				...
				*/
				...
				$data['list'] = $this->db->query('SELECT * FROM product WHERE brand_id="'.$nav->brand_id.'" && (type_gender_id="'.$gender_id.'" || type_gender_id="3") && product_status="publish"'.$price.' '.$sort.' LIMIT '.$this->limit.' offset '.$offset)->result();
				$data['count'] = $this->db->query('SELECT * FROM product WHERE brand_id="'.$nav->brand_id.'" && (type_gender_id="'.$gender_id.'" || type_gender_id="3") && product_status="publish"'.$price)->num_rows();
				if ($data['count']==0) {
					$data['empty'] = '<div class="no-products"><span class="no_available_products">No product available in this category</span></div>';
				} else {
					$data['empty'] = "";
				...
				$config['base_url'] = base_url().'brand/'.str_replace(" ", "-", strtolower($param)).'/'.strtolower($param_type).'/';
				$config['total_rows'] = $data['count'];
