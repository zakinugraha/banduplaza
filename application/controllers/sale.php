...
class Sale extends CI_Controller
{
	...
	public function index()
	{
		...
		$data['title'] = 'Sepatu dan Tas Diskon - Pusat Sepatu dan Tas Terlengkap';
		...
		if ($param!='') {
			if ($param_type=='' || is_numeric($param_type)) {
				...
				$data['list'] = $this->db->query('SELECT * FROM product WHERE product_price_discount>0 && product_status="publish"'.$price.' '.$sort.' LIMIT '.$this->limit.' offset '.$offset)->result();
				$data['count'] = $this->db->query('SELECT * FROM product WHERE product_price_discount>0 && product_status="publish"'.$price)->num_rows();
				if ($data['count']==0) {
				...
				$data['list'] = $this->db->query('SELECT * FROM product WHERE product_price_discount>0 && (type_gender_id="'.$gender_id.'" || type_gender_id="3") && product_status="publish"'.$price.' '.$sort.' LIMIT '.$this->limit.' offset '.$offset)->result();
				$data['count'] = $this->db->query('SELECT * FROM product WHERE product_price_discount>0 && (type_gender_id="'.$gender_id.'" || type_gender_id="3") && product_status="publish"'.$price)->num_rows();
				if ($data['count']==0) {
					$data['empty'] = '<div class="no-products"><span class="no_available_products">No product available in this category</span></div>';
				} else {
					$data['empty'] = "";
				...
				$config['base_url'] = base_url().'sale/'.$param_type.'/';
				$config['total_rows'] = $data['count'];
